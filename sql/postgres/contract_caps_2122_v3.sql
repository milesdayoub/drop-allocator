...
/* Knobs:
   - z: one-sided z for Wilson UCB on new-coupon redemption (1.96=97.5%, 1.645=95%, 1.28=90%)
   - floor_r: floor redemption for new coupons (prevents divide-by-zero / runaway caps)
   ...
   - no_hist_p_ucb: UCB used for new coupons when no history (e.g., 0.25; keep it ≥ no_hist_p_point)
*/
...
WITH params AS (
  SELECT
    ...
    0.03::numeric                       AS floor_r,          -- floor on new-coupon redemption
    ...
    0.25::numeric                       AS no_hist_p_ucb     -- for NEW coupons when no history
),
...
outstanding AS (                   -- live, unredeemed claims ("coupons")
    SELECT contract_address,
...
redeem_rates AS (                  -- point estimate + UCB + floor handling
    SELECT
      ...
      END AS p_point,
      /* One-sided Wilson UCB: used for NEW coupons (knobbed by z) */
      CASE
        WHEN COALESCE(h.assigns_180d,0) = 0
...
budget_frame AS (
    SELECT
      ...
      rr.p_point,
      GREATEST(rr.p_ucb, (SELECT floor_r FROM params)) AS p_eff_new  -- enforce floor for NEW coupons only
    FROM scope s
...
SELECT
    ...
    /* effective UCB used for NEW coupons (after floor) */
    c.p_eff_new        AS p_upper,
    /* safe additional coupons we can mint now:
